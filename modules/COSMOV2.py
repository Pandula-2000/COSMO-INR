...
class RaisedCosineImpulseResponseLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=True, is_first=False,
        ...
        self.theta = nn.Parameter(torch.empty(out_features).uniform_(-np.pi, np.pi))
        self.eps = eps
    ...
    def forward(self, input, t0, c0, theta0):
        ...
        modulation = torch.cos(2 * torch.pi * c0 * lin + theta0)

        rc = (f1 / f2)

        # out = rc*(torch.sin(theta) + torch.cos(theta))
...
class INR(nn.Module):
    def __init__(self, in_features, hidden_features, hidden_layers, activation_parameters, out_features, MLP_configs):
        ...
        # 1. Store a generalized list of parameter ranges instead of hardcoded ones
    ...
    def forward(self, coords):
        ...
        coef = self.prior(gap[..., 0]).view(self.hidden_layers + 1, self.activation_parameters)
        
        # 2. Unbind returns a tuple of N tensors, where N is self.activation_parameters
        raw_params = torch.unbind(coef, dim=1)

        # 3. Apply Sigmoid Projection dynamically to all parameters
        projected_params = []
        ...
        # 4. Dynamically feed parameters into the layer
        for id, lyr in enumerate(self.net):
            # Extract the specific values for layer 'id' across all parameters
