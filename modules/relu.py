...
class INR(nn.Module):
    def __init__(self, in_features, hidden_features, hidden_layers, out_features, ffn_type=None, outermost_linear=True,
        ...
        # Positional Encoding
        if self.pos_encode in encoding.Encoding().encoding_dict.keys():
            self.positional_encoding = encoding.Encoding(self.pos_encode).run(in_features=in_features, pos_encode_configs=pos_encode_configs)
